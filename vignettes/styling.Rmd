...
Adds a background color to group header rows. This is a terminal operation -- it converts the table to gt, so it must be the last step in your pipeline.
...
If you mix gtsummary tables with plain gt tables in the same document, they won't match visually. `theme_gt_compact()` applies the same JAMA compact look to gt tables so everything is consistent. See `vignette("themes")` for side-by-side comparisons and details.
