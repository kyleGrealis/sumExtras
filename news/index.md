...
- [`extras()`](https://www.kyleGrealis.com/sumExtras/reference/extras.md)
  ...
  automatically when p-values are added.
- New `header` parameter in
  [`extras()`](https://www.kyleGrealis.com/sumExtras/reference/extras.md)
  controls the label column header text (default `""`).
- New `symbol` parameter in
  [`clean_table()`](https://www.kyleGrealis.com/sumExtras/reference/clean_table.md)
  and
  [`extras()`](https://www.kyleGrealis.com/sumExtras/reference/extras.md)
  controls the missing value replacement string (default `"---"`).
- [`add_group_colors()`](https://www.kyleGrealis.com/sumExtras/reference/add_group_colors.md)
  now accepts a vector of colors (one per group) in addition to a single
  color for all groups.
...
- Extended list of zero or `NA` value recognition used by `clean_table`.
  This fixes the function previously not recognizing `0 (0)` for Mean
  (SD).
...
- [`clean_table()`](https://www.kyleGrealis.com/sumExtras/reference/clean_table.md)
  ...
  converted to dashes
- `apply_labels_from_dictionary()` improved with clearer implementation
  using traditional for-loop instead of confusing `<<-` side-effect
  assignment
...
- Replaced
  ...
  with traditional for-loop in `apply_labels_from_dictionary()` for
...
- Enhanced
  ...
  documentation with implementation notes about gtsummary internals
...
- [`add_auto_labels()`](https://www.kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
  ...
  addition to `tbl_summary` objects (55bc540)
- [`add_auto_labels()`](https://www.kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
...
- [`add_auto_labels()`](https://www.kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
  ...
  dictionary is found, so it works with {haven}, {labelled}, and other
  packages that set label attributes
- [`add_auto_labels()`](https://www.kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
  uses a fixed label priority: manual labels (from `label = list()`)
  take precedence, then attribute labels, then dictionary labels, then
  variable names
- Added `apply_labels_from_dictionary()` function to set label
  attributes on data from a dictionary for use with ggplot2 4.0+, gt,
  and other label-aware packages
- Label priority is fixed: manual labels \> attribute labels \>
  dictionary labels
...
- [`add_auto_labels()`](https://www.kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
  now falls back to label attributes when no dictionary is found instead
  of erroring
...
- [`extras()`](https://www.kyleGrealis.com/sumExtras/reference/extras.md)
  now warns when called with unsupported table types (tbl_regression,
...
- Added
  [`extras()`](https://www.kyleGrealis.com/sumExtras/reference/extras.md)
  function for one-call gtsummary table styling
- Added
  [`clean_table()`](https://www.kyleGrealis.com/sumExtras/reference/clean_table.md)
