---
title: "sumExtras: Streamlining Table Workflows in Clinical Research with R"
author:
  - name: "Kyle Grealis"
    affiliation: "University of Miami"
date: "May 8, 2026"
date-format: "MMM D, YYYY"
---

```{r}
#| echo: false
suppressPackageStartupMessages({
  library(tidyverse)
  library(sumExtras)
  library(gtsummary)
  library(gt)
})

options(
  sumExtras.auto_labels = FALSE,
  sumExtras.prefer_dictionary = FALSE,
  digits = 4
)
```

# Introduction
 
> **sumExtras**: "gt**sum**mary table **Extras**"


## The plan
::: {.incremental}
+ Apply a JAMA-ready theme
+ Customize missing values with `clean_table()`
+ One-line table formatting with `extras()`
+ Auto-label variables with `add_auto_labels()`
  <!-- + Create a variable dictionary
  + Set package options -->
+ Add variable group headers [with _**style**_ and \
 [**background coloring**]{.coloring}]{ .fragment }
<!-- + Style and color group rows -->
+ Put it all together in one pipeline
:::


## What we'll build

![](img/final_table.png){fig-align="center"}


## Creating a summary table

Results from a simulated study of two chemotherapy agents.

A dataset containing the baseline characteristics of 200 patients who received Drug A or Drug B. Dataset also contains the outcome of tumor response to the treatment.

```{r}
#| echo: true
#| eval: true
gtsummary::trial
```

##

`mutate` the data to simulate missing values

```{r}
#| echo: true
#| eval: true
#| code-line-numbers: "|4,8,17"
dat <- trial |>
  mutate(
    marker = case_when(
      trt == "Drug B" ~ 0,
      .default = marker
    ),
    grade = case_when(
      trt == "Drug B" ~ NA,
      .default = grade
    )
  )
head(dat, 5)
```

## Initial `gtsummary` table

<br>

```{r}
#| output-location: column
dat |>
  tbl_summary(by = trt)
```


## Setting a theme

<br>
<!-- Use `sumExtras::use_jama_theme()` for a more compact table. -->

```{r}
#| output-location: column
# Use a compact theme
sumExtras::use_jama_theme()

dat |>
  tbl_summary(by = trt)
```

::: {.fragment}
`use_jama_theme()` is a wrapper function for \
 `gtsummary::set_gtsummary_theme(theme_gtsummary_compact("jama"))`
:::


## Cleaning the table

![](img/tbl_with_zeros.png){width="45%"}

::: {.fragment}
**THE GOAL:**
Draw attention to complete, non-`NA` values with `clean_table()`.
Replace `0 (0.00, 0.00)`, `0 (NA%)`, and similar missingness with an emdash (`—`).
:::

## Cleaning the table

```{r}
#| eval: false
dat |>
  tbl_summary(by = trt) |>
  clean_table()
```

:::: {.columns}
::: {.column}
Without `clean_table()`
```{r}
#| echo: false
use_jama_theme()
dat |>
  tbl_summary(by = trt)
```
:::

::: {.column}
With `clean_table()`
```{r}
#| echo: false
use_jama_theme()
dat |>
  tbl_summary(by = trt) |>
  clean_table()
```
:::
::::


## Adding labels, *p*-values, and other goodies

```{r}
#| echo: false
#| eval: false
dat |>
  tbl_summary(
    by = trt,
    label = list(
      age ~ "Age at Enrollment (years)",
      marker ~ "Marker Level (ng/mL)",
      stage ~ "T Stage",
      grade ~ "Tumor Grade",
      response ~ "Tumor Response",
      death ~ "Patient Died"
    )
  ) |>
  modify_header(label = "") |>
  bold_labels() |>
  add_p() |>
  bold_p() |>
  add_overall() |>
  clean_table()
```

<!-- ::: {.incremental .smallest}
* Removed "Characteristic" from variable heading
* Added *p*-values
* Bold labels & significant *p*-values
* Added "Overall" column
* Applied `clean_table()`
::: -->

![](img/table-with-goodies-1.png){width=85%}  

This is more like what I want...

## Adding labels, *p*-values, and other goodies
![](img/table-with-goodies-2.png){width=85%}  

Removed "Characteristic" from variable heading

## Adding labels, *p*-values, and other goodies
![](img/table-with-goodies-3.png){width=85%}  

Added *p*-values

## Adding labels, *p*-values, and other goodies
![](img/table-with-goodies-4.png){width=85%}  

Bold labels & significant *p*-values

## Adding labels, *p*-values, and other goodies
![](img/table-with-goodies-5.png){width=85%}  

Added "Overall" column

## Adding labels, *p*-values, and other goodies
![](img/table-with-goodies-6.png){width=85%}  

Applied `clean_table()`


## Adding labels, *p*-values, and other goodies

:::: {.columns}
::: {.column}
```{r}
#| eval: false
dat |>
  tbl_summary(
    by = trt,
    label = list(
      age ~ "Age at Enrollment (years)",
      marker ~ "Marker Level (ng/mL)",
      stage ~ "T Stage",
      grade ~ "Tumor Grade",
      response ~ "Tumor Response",
      death ~ "Patient Died"
    )
  ) |>
  modify_header(label = "") |>
  bold_labels() |>
  add_p() |>
  bold_p() |>
  add_overall() |>
  clean_table()
```
:::

::: {.column}
```{r}
#| eval: false
dat |>
  tbl_summary(
    by = trt,
    label = list(
      age ~ "Age at Enrollment (years)",
      marker ~ "Marker Level (ng/mL)",
      stage ~ "T Stage",
      grade ~ "Tumor Grade",
      response ~ "Tumor Response",
      death ~ "Patient Died"
    )
  ) |>
  extras()
```
:::{.fragment}
Six lines converted to one!
:::

:::

::::


# Labeling hierarchy

::: {.incremental}
* Using `gtsummary::tbl_*`'s `label` parameter
* Dataset label attributes
* Using `sumExtras` auto-labeling
:::


## Creating a variable dictionary

A `dictionary` is a data frame with `variable` and `description` columns.

:::: {.columns}
::: {.column width="55%"}
```{r}
#| echo: true
#| code-line-numbers: "|2"
dictionary <- tibble::tribble(
  ~variable,    ~description,
  "trt",        "Chemotherapy Treatment",
  "age",        "Age (years)",
  "marker",     "Marker Level (ng/mL)",
  "stage",      "T Stage",
  "grade",      "Tumor Grade",
  "response",   "Tumor Response",
  "death",      "Patient Died"
)
```
:::
::: {.column width="45%"}
```{r}
dictionary
```
:::
::::

::: {.fragment}
::: {.callout-note}
Column names are case-*in*sensitive.
:::
:::


## Using `add_auto_labels()` {.col-45-55}

With a `dictionary` in scope, labels are applied automatically.

```{r}
#| output-location: column
#| code-line-numbers: "|1-3|1-3,8"
options(
  sumExtras.prefer_dictionary = TRUE
)

dat |>
  tbl_summary(by = trt) |>
  extras() |>
  add_auto_labels()
```

::: {.fragment}
No more writing `label = list(...)` for every table!
:::


## Streamline with options

<br>

:::: {.columns}

::: {.column width="48%"}

```{r}
#| code-line-numbers: "|1-4|2,8,9"
#| eval: false
options(
  sumExtras.auto_labels = TRUE,
  sumExtras.prefer_dictionary = TRUE
)

dat |>
  tbl_summary(by = trt) |>
  extras() #|> 
  # add_auto_labels()
```

::: {.fragment .smallest}
**Label hierarchy:** `label` parameter > dictionary > attributes.  
Without `prefer_dictionary`: `label` > attributes > dictionary.
:::

:::

::: {.column width="52%"}

```{r}
#| echo: false
options(
  sumExtras.auto_labels = TRUE,
  sumExtras.prefer_dictionary = TRUE
)

dat |>
  tbl_summary(by = trt) |>
  extras()
```
:::

::::



# Grouping variables & adding color

## Group variables {.col-45-55}

```{r}
#| output-location: column
#| code-line-numbers: "|12"
dat |>
  tbl_summary(by = trt) |>
  extras() |>
  add_variable_group_header(
    header = "Demographics",
    variables = age:stage
  ) |>
  add_variable_group_header(
    header = "Clinical Measures",
    variables = grade:ttdeath
  ) |>
  add_group_styling()
```

::: {.fragment}
`add_group_styling()` uses **bold** & *italics* by default and restores
left-justified variable indentation.
:::


## Add grouping colors {.col-45-55}

```{r}
#| output-location: column
#| code-line-numbers: "|13-15"
dat |>
  tbl_summary(by = trt) |>
  extras() |>
  add_variable_group_header(
    header = "Demographics",
    variables = age:stage
  ) |>
  add_variable_group_header(
    header = "Clinical Measures",
    variables = grade:ttdeath
  ) |>
  add_group_styling() |>
  add_group_colors(
    color = c("#E1EAE2", "#FFF9E6")
  )
```

::: {.fragment}
**NOTE:** `add_group_colors()` CONVERTS the table to a `gt` table to apply the tweaks! Use this function as the *last* step in the pipeline.
:::


## Reduced code!

:::: {.columns}
::: {.column}
```{r}
#| eval: false
#| code-line-numbers: "|4-11,13-18"
dat |>
  tbl_summary(
    by = trt,
    label = list(
      age ~ "Age (years)",
      marker ~ "Marker Level (ng/mL)",
      stage ~ "T Stage",
      grade ~ "Tumor Grade",
      response ~ "Tumor Response",
      death ~ "Patient Died"
    )
  ) |>
  modify_header(label = "") |>
  bold_labels() |>
  add_p() |>
  bold_p() |>
  add_overall() |>
  clean_table() |>
  add_variable_group_header(
    header = "Demographics",
    variables = age:stage
  ) |>
  add_variable_group_header(
    header = "Clinical Measures",
    variables = grade:ttdeath
  ) |>
  add_group_styling() |>
  add_group_colors(
    color = c("#E1EAE2", "#FFF9E6")
  )
```
:::

::: {.column}

```{r}
#| eval: false
#| code-line-numbers: "|3"
dat |>
  tbl_summary(by = trt) |>
  extras() |>
  add_variable_group_header(
    header = "Demographics",
    variables = age:stage
  ) |>
  add_variable_group_header(
    header = "Clinical Measures",
    variables = grade:ttdeath
  ) |>
  add_group_styling() |>
  add_group_colors(
    color = c("#E1EAE2", "#FFF9E6")
  )
```
::: {.fragment}
We just cut our code **in half**... 
and that was only one table!
:::

:::

::::


## And this is what we built!

![](img/final_table.png){fig-align="center"}



## `extras()` parameters

```{r}
#| eval: false
#| code-line-numbers: "|2,5-8"
extras(
  # gtsummary defaults:
  # Show the p-value column?
  pval = TRUE,        # or FALSE
  # Show the Overall column?
  overall = TRUE,     # or FALSE
  # Position the Overall column last?
  last = FALSE,       # or TRUE

  # sumExtras options:
  header = "",        # removes "Characteristic" header
  symbol = "—",       # display value for clean_table()
  .args = NULL,       # list override the default (next slide)
  .add_p_args = NULL  # gtsummary::add_p() arguments
)
```

::: {.fragment}
`pval`, `overall`, and `last` reuse `gtsummary`'s defaults for:  
* `add_p()`  
* `add_overall()`
:::



## `extras()` using `.args` parameter

Create reusable `extras()` arguments to override defaults:

```{r}
#| output-location: column
#| code-line-numbers: "|1-5|2,4|9"
extra_args <- list(
  pval = FALSE,       # default was TRUE
  overall = TRUE,
  last = TRUE         # default was FALSE
)

trial |> 
  tbl_summary(by = trt) |> 
  extras(.args = extra_args)
```


## `extras()` using `.add_p_args`

Create custom *p*-value testing & styling

```{r}
#| eval: false
#| code-line-numbers: "|1-5|10"
p_args <- list(
  test = list(age ~ "t.test", marker ~ "t.test"),
  pvalue_fun = ~ gtsummary::style_pvalue(
    .x, digits = 2
  )
)

trial |> 
  tbl_summary(by = trt) |> 
  extras(.add_p_args = p_args)
```


## Match gtsummary tables to gt tables

If you use `gtsummary` and `gt`, `sumExtras` can help with that!

:::: {.columns}

::: {.column}

```{r}
trial |>
  tbl_summary(
    by = trt,
    include = c(age, grade, marker)
  ) |>
  extras()
```
:::


::: {.column}

```{r}
#| code-line-numbers: "|5"
trial |>
  select(trt, age, grade, marker) |>
  head(10) |>
  gt() |>
  theme_gt_compact()
```
:::

::::



## `sumExtras` supports most `gtsummary` tables

with either partial or full features available.

* `tbl_summary()`
* `tbl_svysummary()`
* `tbl_regression()`
* `tbl_uvregression()`
* `tbl_strata()`
* `tbl_merge()`


## THANK YOU! 😄

:::{.absolute bottom="30%" left="-8%"}
<span style="font-size: 50px;"><strong>Kyle Grealis, MS</strong></span>

**GitHub**: https://github.com/kyleGrealis

**Email**: kyle@kyleGrealis.com

**Slides**: https://slides.kyleGrealis.com/sumExtras

**Package**: https://www.kyleGrealis.com/sumExtras
:::

![](img/kyle.jpg){.styled-image .absolute bottom="30%" right="1%" height="auto" width="190"}

