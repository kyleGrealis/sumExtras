...
Draw attention to complete, non-`NA` values with `clean_table()` by replacing `0 (0.00, 0.00)`, `0 (NA%)`, and other similar values with an emdash (`—`).
...
The `dictionary` is found by `sumExtras` but manual labelling inside the
`tbl_*` function **always** wins!
