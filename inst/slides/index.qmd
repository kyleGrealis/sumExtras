...
```{r}
#| echo: false
attr(dat$age, "label") <- "Age FROM ATTRIBUTE (years)"
dat |>
  tbl_summary(by = trt) |>
  extras()
```
...
Declare the variables labels within the `tbl_*` function:
...
The `label` argument from the `tbl_*` function **always** wins!
...
dat |>
  tbl_summary(
    ...
    label = list(
      ...
      marker ~ "Marker Level LABEL (ng/mL)",
      stage ~ "T Stage LABEL",
      grade ~ "Tumor Grade LABEL",
      response ~ "Tumor Response LABEL",
...
Remove the manual labels and use the `dictionary`.
...
::: {.fragment}
UH OH! Where are my `dictionary` labels??
:::
