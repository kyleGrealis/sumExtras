...
tables and plots is labeling variables with human-readable descriptions.
Instead of manually typing labels for every variable in every table and
plot, sumExtras provides a unified labeling system that works across
...
sumExtras uses R’s built-in [`attr()`](https://rdrr.io/r/base/attr.html)
function to work with variable labels - the same labeling approach used
by haven, Hmisc, labelled, and ggplot2 4.0+. This means labels work
seamlessly across the R ecosystem, whether you’re creating tables with
gtsummary, plots with ggplot2, or outputs with gt.

### Understanding Label Attributes

Labels in R are stored as attributes on individual variables. Here’s
...
The key is that if labels are there, sumExtras can use them. This
flexibility means one labeling system works everywhere - no matter where
your data came from or how it was prepared.

## Creating a Data Dictionary

A data dictionary serves dual purposes: it documents your variables and
provides labels for automatic application. The dictionary is simply a
...
1.  **Store externally** - Keep the dictionary as a CSV file or database
    table
2.  **Load once** - Read it at the beginning of your analysis script
3.  **Version control** - Track changes to labels over time
4.  **Share widely** - Use the same dictionary across all project
    analyses
...
This centralizes your variable documentation and ensures consistency
across all outputs.
...
The
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
function is designed to be flexible and intelligent. It can work with
dictionaries, pre-labeled data, or both, and it always respects manual
overrides.

### Method 1: Pass Dictionary Explicitly

The most straightforward approach is to pass your dictionary directly to
the function:
...
This approach is explicit and clear - you can see exactly where the
labels are coming from.

### Method 2: Automatic Discovery

If you have a `dictionary` object in your environment,
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
will find it automatically without needing to pass it explicitly:
...
The first time
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
finds your dictionary automatically in a session, you’ll see a friendly
message: “Auto-labeling from ‘dictionary’ object in your environment
(this message will only show once per session)”. This confirms that your
dictionary was found and is being used.

This is particularly convenient when working in an R Markdown or Quarto
document where your dictionary is defined once at the top.

### Method 3: Working with Pre-Labeled Data

If your data already has label attributes (from packages like haven,
labelled, or set manually),
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
...
This is especially useful when working with data imported from SAS,
SPSS, or Stata files that already contain variable labels.

### Manual Overrides Always Win

No matter where labels come from (dictionary or attributes), manual
labels specified in your
[`tbl_summary()`](https://www.danieldsjoberg.com/gtsummary/reference/tbl_summary.html)
call always take precedence:
...
This gives you complete control: use automated labeling for most
variables, but override specific ones when needed.
...
Labels are applied to both the predictors and the outcome variable,
making regression output immediately readable.

## Setting Label Attributes with `apply_labels_from_dictionary()`

While
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
works directly on gtsummary tables,
[`apply_labels_from_dictionary()`](https://kyleGrealis.com/sumExtras/reference/apply_labels_from_dictionary.md)
takes a different approach: it sets label attributes on your data frame.
This enables cross-package workflows where the same labels work in both
gtsummary tables and ggplot2 visualizations.
...
Now this labeled data can be used anywhere R label attributes are
recognized.
...
Notice we don’t need to pass the dictionary - the labels are already
stored as attributes on the data.

### Use Labeled Data in ggplot2

With ggplot2 version 4.0 and later, label attributes are automatically
used for axis and legend labels:
...
![](labeling_files/figure-html/unnamed-chunk-12-1.png)

No need to manually specify
[`labs()`](https://ggplot2.tidyverse.org/reference/labs.html) - the
labels from your dictionary are applied automatically to the x-axis,
y-axis, and legend.
...
available,
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
needs to decide which one to use. You control this with a global option.
...
`sumExtras.preferDictionary` option to `TRUE`. This is useful when
you’re actively maintaining a master dictionary and want it to be the
...
- **`FALSE` (default)**: You’re importing labeled data from
  SAS/Stata/SPSS and want to preserve those labels
- **`TRUE`**: You maintain a master dictionary and want it to override
  any existing labels

Remember: manual labels set via `label = list(...)` in
[`tbl_summary()`](https://www.danieldsjoberg.com/gtsummary/reference/tbl_summary.html)
always win, regardless of this option.
...
and
[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)
enables this seamlessly.

### Complete Workflow Example

Here’s a realistic workflow showing how one dictionary serves both
gtsummary tables and ggplot2 visualizations:
...
![](labeling_files/figure-html/unnamed-chunk-15-1.png)

Notice how the axis labels, legend titles, and facet labels are
automatically pulled from your dictionary - no manual
[`labs()`](https://ggplot2.tidyverse.org/reference/labs.html) calls
needed! This workflow ensures perfect consistency between your tables
and plots.
...
Here’s a comprehensive example showing how the labeling system
streamlines a typical analysis workflow:
...
![](labeling_files/figure-html/unnamed-chunk-16-1.png)
...
![](labeling_files/figure-html/unnamed-chunk-16-2.png)
...
![](labeling_files/figure-html/unnamed-chunk-16-3.png)

This workflow demonstrates the power of the labeling system: define your
labels once in a dictionary, apply them to your data, then create as
many tables and plots as you need with consistent, professional labeling
throughout.
...
Note: New variables created with
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) won’t
have labels unless you set them explicitly or add them to your
dictionary.

### Working with Multiple Dictionaries

For large projects, you might maintain separate dictionaries for
...
1.  **Variable names match exactly** - Dictionary Variable column must
    match data exactly (case-sensitive)
2.  **Dictionary in scope** - If using auto-discovery, ensure dictionary
    object exists
3.  **Manual labels present** - Manual labels always override automatic
    ones
4.  **Attribute structure** - Use `str(your_data)` to verify label
    attributes exist
...
1.  **Name the object ‘dictionary’** - Auto-discovery looks for an
    object named exactly “dictionary”
2.  **Pass explicitly** - Use `add_auto_labels(dictionary = my_dict)` if
    named differently
3.  **Check environment** - Ensure dictionary is loaded in the current
    session
...
2.  **Use preferDictionary option**: Set
    `options(sumExtras.preferDictionary = TRUE)` to reverse
3.  **Manual override**: Use `label = list(var ~ "Custom")` in
    [`tbl_summary()`](https://www.danieldsjoberg.com/gtsummary/reference/tbl_summary.html)
    for specific variables

## Summary

The sumExtras labeling system provides a unified approach to variable
labeling across your entire analysis:

- **[`add_auto_labels()`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md)** -
  Smart labeling for gtsummary tables (uses dictionary or attributes)
- **[`apply_labels_from_dictionary()`](https://kyleGrealis.com/sumExtras/reference/apply_labels_from_dictionary.md)** -
  Set labels as data attributes for cross-package workflows
- **One dictionary** - Consistent labels across tables, plots, and
  outputs
- **Flexible priority** - Control whether attributes or dictionary takes
  precedence
...
- [`vignette("styling")`](https://kyleGrealis.com/sumExtras/articles/styling.md) -
  Advanced table styling and formatting
- [`?add_auto_labels`](https://kyleGrealis.com/sumExtras/reference/add_auto_labels.md) -
  Function documentation
- [`?apply_labels_from_dictionary`](https://kyleGrealis.com/sumExtras/reference/apply_labels_from_dictionary.md) -
  Function documentation

The labeling system is designed to save you time while ensuring
consistency. Define your labels once, use them everywhere, and let
sumExtras handle the rest.
